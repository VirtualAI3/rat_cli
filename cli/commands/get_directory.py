...
class GetDirectoryCommand(CommandSet):
    ...
    get_directory_parser.add_argument("--dest", type=str, required=True, help="Ruta de destino en el servidor")
    ...
    def do_get_directory(self, args):
        ...
        # Enviar comando
        try:
            directory_handler = DirectoryHandler(self._cmd.client_manager)
            if directory_handler.enviar_comando_solicitar_directorio(args.source, args.dest, cliente_id):
                console.print(f"[bold green]Comando enviado: solicitar directorio '{args.source}' hacia '{args.dest}'[/bold green]")
            else:
