...
class GetFilesByExtensionCommand(CommandSet):
    ...
    get_files_parser.add_argument("--dest", type=str, required=False, help="Ruta de destino en el servidor (Opcional). Ya existe una ruta por defecto.")
    ...
    def do_get_files_by_extension(self, args):
        ...
        # Validar directorio
        if not validate_path(args.dir):
            console.print("[bold red]Error: La ruta de directorio debe ser válida.[/bold red]")
            return
        ...
        # Enviar comando
        try:
            console.print(f"[bold green]Comando enviado: solicitar archivos con extensión '{args.ext}' desde '{args.dir}'[/bold green]")
            resultado = self._cmd.client_manager.enviar_comando_archivos_por_extension(args.dir, args.ext, args.dest, cliente_id)
